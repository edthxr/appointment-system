...
export default async function AdminDashboard() {
  ...
  return (
    <div className="animate-in fade-in duration-1000">
      <div className="mb-12">
        <h1 className="text-5xl font-display font-black text-foreground tracking-tighter mb-2">{t('dashboard.title')}</h1>
        <p className="text-[13px] font-black text-accent uppercase tracking-[0.3em]">{t('dashboard.subtitle').replace('{name}', activeClinic.name)}</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="card-luxury p-8">
          <div className="flex items-center justify-between mb-6">
            <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.2em]">{t('dashboard.total_appointments')}</p>
          ...
          <p className="text-[11px] text-green-600 font-bold mt-4 flex items-center gap-1">
            ...
            <span className="opacity-60 uppercase tracking-widest text-[9px]">{t('dashboard.vs_last_month')}</span>
          </p>
        </div>

        <div className="card-luxury p-8 border-accent/10">
          <div className="flex items-center justify-between mb-6">
            <p className="text-[10px] font-black text-accent uppercase tracking-[0.2em]">{t('dashboard.pending_requests')}</p>
          ...
          <p className="text-[11px] text-foreground-muted font-bold mt-4 uppercase tracking-widest opacity-60">{t('dashboard.awaiting_confirmation')}</p>
        </div>

        <div className="card-luxury p-8">
          <div className="flex items-center justify-between mb-6">
            <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.2em]">{t('dashboard.gross_revenue')}</p>
            <div className="w-10 h-10 rounded-2xl bg-muted flex items-center justify-center border border-border-ios/40 text-accent">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-[11px] text-green-600 font-bold mt-4 flex items-center gap-1">
            ...
            <span className="opacity-60 uppercase tracking-widest text-[9px]">{t('dashboard.calculated_yield')}</span>
          </p>
        </div>

        <div className="card-luxury p-8">
          <div className="flex items-center justify-between mb-6">
            <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.2em]">{t('dashboard.cancelled_orders')}</p>
          ...
          <p className="text-[11px] text-red-500/60 font-bold mt-4 uppercase tracking-widest opacity-80">{t('dashboard.requires_review')}</p>
        </div>
      </div>

      <div className="mt-20 grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2">
           <h2 className="text-2xl font-display font-black mb-10 text-foreground uppercase tracking-tighter">{t('dashboard.performance_log')}</h2>
           <div className="card-luxury min-h-[400px] flex flex-col items-center justify-center border-dashed border-2 bg-muted/20 opacity-80">
              ...
              <p className="text-foreground-muted text-[13px] font-black uppercase tracking-[0.3em] italic opacity-40 text-center px-8">
        ...
        <div className="space-y-10">
           <h2 className="text-2xl font-display font-black mb-10 text-foreground uppercase tracking-tighter">{t('dashboard.market_pulse')}</h2>
           <div className="card-luxury p-8">
              <p className="text-[10px] font-black text-foreground-muted uppercase tracking-[0.2em] mb-8">{t('dashboard.service_popularity')}</p>
              <div className="space-y-6">
                 {stats.popularServices.length === 0 ? (
                    <p className="text-[11px] text-foreground-muted font-bold italic opacity-60 uppercase tracking-widest py-8 text-center border border-dashed border-border-ios/40 rounded-2xl">{t('dashboard.insufficient_data')}</p>
                 ) : (
           ...
           <div className="card-luxury p-8 bg-accent/5 border-accent/10">
              <p className="text-[10px] font-black text-accent uppercase tracking-[0.2em] mb-4">{t('dashboard.quick_optimization')}</p>
              <p className="text-[12px] font-medium text-foreground-muted leading-relaxed mb-6 opacity-80 italic">
                "{t('dashboard.optimization_tip')}"
              </p>
